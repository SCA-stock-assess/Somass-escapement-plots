...
# -----------------------------------------------------------------------------
StampHistPadded <- map2(Stampfiles$file, Stampfiles$year, load_Stamp_year) |>
  bind_rows() |>
  rename(
    date      = `Review Date`,
    co_mark   = `Co  Mark`,
...
# -----------------------------------------------------------------------------
#Stamp River — current year loader
#   Note: column names with spaces (`Co  NoMark`, `Co  Mark`) need backticks —
#   bare `Co NoMark` is invalid R syntax. Matches the two-space naming
#   convention used in the historical Sproat loader (section 4) — confirm the
...
StampCurrent <- read_xlsx(
  ...
  mutate(
    date          = as.Date(Date),
    julian        = safe_julian(date),
    CohoTotal     = as.numeric(Coho),
    CohoNotMarked = as.numeric(`Co  NoMark`),
    CohoMarked    = as.numeric(`Co  Mark`),
    year          = as.integer(CURRENT_YEAR)
  ) |>
  filter(!is.na(date)) |>
  arrange(date) |>
  mutate(
    cum_count        = cumsum(replace_na(CohoTotal, 0)),
    cum_count_mark   = cumsum(replace_na(CohoMarked, 0)),
    cum_count_nomark = cumsum(replace_na(CohoNotMarked, 0))
  ) |>
  select(year, date, julian,
