...
forecast_colours <- c(
  "Quartile 1" = "#d73027",    # red        — lowest survival
  "Quartile 2" = "#fdae61",    # orange     — below average
  "Quartile 3" = "#74c476",    # mid green  — above average
  "Quartile 4" = "#238b45"     # dark green — highest survival
)
...
aug25_vline <- list(
  ...
  annotate("text", x = 238, y = -Inf, label = "August 25",
           hjust = -0.1, vjust = -0.5, color = "black", size = 3.5)
)

ribbon_light <- "#d0e8f5"    # below 25th percentile — behind historical norm
ribbon_mid   <- "#74a9cf"    # 25th to 60th percentile — typical range
ribbon_dark  <- "#2166ac"    # above 60th percentile — ahead of historical norm
...
p_bottom <- ggplot(forecast_df, aes(x = category, y = value, fill = category)) +
  ...
  geom_col(data = filter(forecast_df, category == forecast_category),
           ...
           fill = forecast_colour, width = 0.7, colour = "black", linewidth = 1) +
  annotate("text", x = forecast_category, y = 0.5,
           label = "◄ Current", hjust = -0.1, fontface = "bold") +
...
build_ribbon_plot <- function(hist_data, hist_median_col, current_col, y_label) {
  ggplot() +
    ...
    geom_line(data = hist_data,
              aes(x = julian, y = .data[[hist_median_col]]),
    ...
    annotate("text",
             ...
             label  = paste0(CURRENT_YEAR, " (total Co)"),
             colour = "black", hjust = -0.1, size = 3) +
    aug25_vline +
    scale_y_continuous(name = paste("Sproat —", y_label)) +
    x_scale +
    theme_classic()
}
...
p_nomark  / p_bottom + plot_layout(heights = c(4, 1)) #❌ p_mark — not valid ti-nseason
